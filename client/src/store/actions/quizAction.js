import axiosApp from '../../axios/axiosApp'
import { ADD_NEW_BLOCKS, CLEAR_COURSES, quizSET_CURRENT_SQUARE, SET_CURRENT_ROUND } from './actionTypes'

// const urlImg =
//   'data:image/png;base64,'
// courses: [
//     {name: "react", img: urlImg,  aswers: [ { aswer1: "how to do " }, { aswer2: "yeah" }]   },
//     {name: "react", img: urlImg,  aswers: [ { aswer1: "how to do " }, { aswer2: "yeah" }]   },
//     {name: "react", img: urlImg,  aswers: [ { aswer1: "how to do " }, { aswer2: "yeah" }]   },
// ]

// const testTheory =
//   ' метод нахождения локального экстремума (минимума или максимума) функции с помощью движения вдоль градиента. Для минимизации функции в направлении градиента используются методы одномерной оптимизации, например, метод золотого сечения. Также можно искать не наилучшую точку в направлении градиента, а какую-либо лучше текущей.'

// let fakeData = [
//     {
//         id: 0,
//         name: "Математика",
//         img: urlImg,
//         courseInfo: "Это курс по стандартной математики",
//         courseBlocks: [
//             {   blokName: "Деф уравнения",
//                 // blockAswersAndTheory: [
//                 //     { isAnswer: true,   answer: "5 + 5",  rightOption: 1,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 1 }, {aswer3: 322 }] },
//                 //     { isAnswer: false,  theoryName: "Градиентный спуск", theory: testTheory } ,
//                 //     { isAnswer: true,   answer: "10 + 5",  rightOption: 3,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 15 }, {aswer3: 8 }]  },
//                 // ]
//             },
//         ]
//     },

//     {
//         id: 1,
//         name: "Высшая математика",
//         img: urlImg,
//         courseInfo: "Это курс по высшей математики",
//         courseBlocks: [

//             {   blokName: "Радном",
//                 // blockAswersAndTheory: [
//                 //     { isAnswer: true,   answer: "5 + 5",  rightOption: 1,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 1 }, {aswer3: 322 }] },
//                 //     { isAnswer: false,  theoryName: "Градиентный спуск", theory: testTheory } ,
//                 //     { isAnswer: true,   answer: "10 + 5",  rightOption: 3,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 15 }, {aswer3: 8 }]  },
//                 // ]
//             },

//             // {  blokName: "линейная алгебра", blockInfo: "Теория блока", questions: [ { question: "5 + 5", rightAswer: 1, blockAswers: [ { aswer1: 10 }, { aswer2: 2 }, {aswer3: 1 } ]  }  ]      },
//             // { blokName: "какая-то рандомная тема этого блока", blockInfo: "Теория блока", questions: [ { question: "5 + 5", rightAswer: 1, blockAswers: [ { aswer1: 10 }, { aswer2: 2 }, {aswer3: 1 } ]  }  ]      }
//             {   blokName: "линейная алгебра",
//                 // blockAswersAndTheory: [
//                 //     { isAnswer: true,   answer: "5 + 5",  rightOption: 1,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 1 }, {aswer3: 322 }] },
//                 //     { isAnswer: false,  theoryName: "Градиентный спуск", theory: testTheory } ,
//                 //     { isAnswer: true,   answer: "10 + 5",  rightOption: 3,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 15 }, {aswer3: 8 }]  },
//                 // ]
//             },

//                 {   blokName: "какая-то рандомная тема этого блока",
//                 // blockAswersAndTheory: [
//                 //     { isAnswer: true,   answer: "5 + 5",  rightOption: 1,  options: [{ aswer1: 10 }, { aswer2: 2 }, {aswer3: 1 }, {aswer3: 322 }] },
//                 //     { isAnswer: false,  theoryName: "Градиентный спуск", theory: testTheory } ,
//                 //     { isAnswer: false,  theoryName: "Градиентный спуск", theory: testTheory } ,
//                 //     { isAnswer: false,  theoryName: "Градиентный спуск", theory: testTheory } ,
//                 // ]
//             },
//         ]
//     },
// ]

export const getDataFromQuizes = (courseId, token) => {
  // let addingData = fakeData.filter( item => item.id === id )
  // console.log("addin data", addingData )
  // return async dispatch => {
  //     // dispatch( addNewQuizesBLock(fakeData) )
  //     try {
  //         let response = await axiosApp.get(`${courseName}`)

  //         let blocks = response.data
  //         console.log( blocks )
  //         // let blocks = fakeData

  //         dispatch( addNewQuizesBLock(blocks) )
  //     } catch (error) {

  //         console.error("QUIZ Block error", error)
  //     }
  // }

  // NEW POST VERTION

  return async (dispatch) => {
    try {
      await axiosApp.get(`/course/${courseId}?token=${token}`).then((res) => {
        const blocks = res.data.data
        if (res.data.redirect) {
        } else {
          dispatch(addNewQuizesBLock(blocks, res.data.name))
        }
      })
    } catch (error) {
      console.error('QUIZ Block error', error)
    }
  }
}

const addNewQuizesBLock = (data, courseName) => {
  return {
    type: ADD_NEW_BLOCKS,
    data,
    courseName,
  }
}

export const clearBlock = () => {
  return {
    type: CLEAR_COURSES,
  }
}

export const setCurrentRound = (course) => {
  return {
    type: SET_CURRENT_ROUND,
    course,
  }
}

export const setCurrentSquare = (squareId) => {
  return {
    type: quizSET_CURRENT_SQUARE,
    squareId,
  }
}
